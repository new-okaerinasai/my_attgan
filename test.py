...
def sample_graph():
    ...
    for n in sess.graph.as_graph_def().node:
        print(n.name)
    for var_name in sess.graph.get_operations():
        tensor = var_name.name + ":0"
        if not tensor.startswith("generator/xa") and not tensor.startswith("generator/b_") and not tensor.startswith("generator/xb"): 
            try:
                sess.run(tensor)
                print(sess.graph.get_tensor_by_name(tensor))
                tf.add_to_collection(tf.GraphKeys.VARIABLES, sess.graph.get_tensor_by_name(tensor))
            except:
                continue
    return run
...
xa = sess.graph.get_tensor_by_name("generator/xa:0")
b_ = sess.graph.get_tensor_by_name("generator/b_:0")
xb = sess.graph.get_tensor_by_name("generator/xb:0")

xa = tf.saved_model.utils.build_tensor_info(xa)
b_ = tf.saved_model.utils.build_tensor_info(b_)
xb = tf.saved_model.utils.build_tensor_info(xb)
print(xa, b_, xb)

prediction_signature = tf.saved_model.signature_def_utils.build_signature_def(
        inputs={"image": xa, "attrs": b_},
        outputs={"output": xb},
